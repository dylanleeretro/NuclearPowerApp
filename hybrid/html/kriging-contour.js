...
function kriging_contour(featureCollection,weight,krigingParams,breaks,local_extent,clip_geom){
    ...
    if(clip_geom){
        let clip_features=contourFeatureCollection.features.map(feature=>{
			let turfClip = intersect(feature,clip_geom);
			if (turfClip !== null) {
				turfClip.properties = feature.properties;
			}
			return turfClip;
        });
        contourFeatureCollection.features=clip_features.filter(d =>d);
    }
