...
var kriging = function() {
	...
	*/
	...
	kriging.gridModified = function(polygons, variogram, width, seapoints) {
		...
		var seaN = seapoints.length;
		var A = Array(seaN);
		for(i=0;i<seaN;i++) A[i] = 0;
		
		console.log(seaN)
		console.log(seapoints[seaN-1])
		console.log(seapoints[0][1])
		console.log(seapoints[131441])
		for(i=0;i<seaN-1;i++) {
			xtarget=seapoints[i][0]
			ytarget=seapoints[i][1]
			A[i] = kriging.predict(xtarget,
							  ytarget,
							  variogram);			
		}			
	...
	kriging.plotModidied = function(canvas, grid, xlim, ylim, colors,seapoints) {
		// Clear screen 
		var ctx = canvas.getContext("2d");
		ctx.clearRect(0, 0, canvas.width, canvas.height);
		...
		var wy = Math.ceil(grid.width*canvas.height/(ylim[1]-ylim[0]));
		console.log(n)
		console.log(seapoints.length)

		for(i=0;i<n-1;i++) {
			if(grid[i]==undefined) continue;
			x = canvas.width*(seapoints[i][0]-xlim[0])/range[0];
			y = canvas.height*(1-(seapoints[i][1]-ylim[0])/range[1]);
			z = (grid[i]-grid.zlim[0])/range[2];
			if(z<0.0) z = 0.0;
			if(z>1.0) z = 1.0;
